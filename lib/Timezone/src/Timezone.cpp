 ...
 * daylight time.                                                       *
 *----------------------------------------------------------------------*/
Timezone::Timezone(TimeChangeRule stdTime) : m_dst(stdTime), m_std(stdTime) {
  initTimeChanges();
}
 ...
 *----------------------------------------------------------------------*/
Timezone::Timezone(int address) { readRules(address); }
#endif
...
time_t Timezone::toTime_t(TimeChangeRule r, int yr) {
  ...
  // calculate first day of the month, or for "Last" rules, first day of the
  // next month
  tmElements_t tm;
  tm.Hour = r.hour;
  tm.Minute = 0;
  ...
  // add offset from the first of the month to r.dow, and offset for the given
  // week
